...
class LINCSSimilarData {
    ...
    public function getLincsDataset($lincsid,$count) {
		...
		if (substr(php_uname(), 0, 7) == "Windows") {
                ...
                $lincsDataset->setDataItemTitle($dataLincs["_source"]["dataItem"]["title"]);
                $lincsDataset->setDataResourceName($dataLincs["_source"]["dataResource"]["name"]);
                $lincsDataset->setCellName($dataLincs["_source"]["rawData"]["Cell Name"]);
