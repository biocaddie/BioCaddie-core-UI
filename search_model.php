...
public function generate_filter(){
        ...
        /*echo '<pre>';
        echo print_r($this->filter_fields);
        echo '</pre>';
        ...
        foreach(array_keys($this->filter_fields) as $key){  
         $filter_field = $key;
            //$filter_value = $this->filter_fields[$key][0];
         $new_filter_values = [];
    ...
    public function generate_sort(){
        $sort=[];
        if($this->sort=='relevance'||$this->sort==''){
            $sort = [];
        }
        else{
...
/*$search_pdb = new ElasticSearch();
$search_pdb->search_fields = ['title','accession'];
$search_pdb->facets_fields = ['organism','Type'];
$search_pdb->query = 'cancer';
$search_pdb->es_index = 'geo';
$search_pdb->es_type = 'array_express';
$search_pdb->filter_fields=[];
//$search_pdb->filter_fields = [ 'entry_type' => 'series'];
//                               'citation_year:2009' => 'on' ];
//$search_pdb->filter_fields = [ 'Type'=>["\"transcription profiling by array, comparative genomic hybridization by array\""]];
$search_pdb->filter_fields = ['organism'=>["Bos taurus, Homo sapiens"]];
$body = $search_pdb->generate_search_body();
echo '<pre>';
echo print_r($body);
echo '<pre>';
